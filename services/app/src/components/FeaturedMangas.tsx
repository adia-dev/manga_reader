...
const FeaturedMangas = (props: Props) => {
  ...
  async function getMangaInformation() {
    ...
    const mappedResults = await Promise.all(
      data.map(async (manga: any) => {
        ...
        const description = removeStringAfterDelimiter(
          manga.attributes.description.en ??
            getRandomProperty(data.attributes.description) ??
            "",
          "---"
        );
  ...
  return (
    <div className="w-10/12 mx-auto z-10 h-[400px] overflow-hidden relative rounded-xl mb-8">
      <div
        className="w-full h-full scale-105 absolute left-0 top-0 -z-10"
        style={{
          backgroundImage:
            results.length > 0
              ? `url(${results[currentMangaIndex].cover})`
              : "",
      ...
      <div className="static p-10">
        ...
        <p className="text-4xl font-bold text-white py-3">
          {results.length > 0 ? results[currentMangaIndex].title : ""}
        </p>
        <p className="text-white text-sm w-2/3 h-[125px]">
          {results.length > 0 ? results[currentMangaIndex].description : ""}
        </p>

        <div className="flex items-center space-x-2 py-3">
          {results.length > 0
            ? results[currentMangaIndex].themes.map((theme: any, key: any) => (
                <div
                  key={key}
        ...
        <div
          draggable="true"
          className="absolute right-10 top-10 w-40 h-60 hover:animate-pulse hover:transform hover:-skew-y-3 hover:scale-110 transition-all duration-500 cursor-pointer"
        >
          <Link to={results.length ? results[currentMangaIndex].link : "/"}>
            ...
            <div className="absolute top-0 left-0 w-40 h-60 bg-gray-100 rounded-xl shadow-2xl  overflow-hidden">
              <img
                src={results.length > 0 ? results[currentMangaIndex].cover : ""}
                alt=""
        ...
        <div className="flex items-center space-x-2 mt-10">
          {/* <Link to={results.length ? results[currentMangaIndex].link : "/"}>
            <button className="rounded-xl bg-dark-tertiary text-dark-primary px-4 py-2 w-24 hover:scale-105 transition duration-500">
              Read
            </button>
          </Link> */}
          <Link to={results.length ? results[currentMangaIndex].link : "/"}>
            <button className="rounded-xl text-dark-primary bg-dark-tertiary px-4 py-2 min-w-24 whitespace-nowrap hover:scale-105 transition duration-500">
              View Info
            </button>
          </Link>
        </div>
      ...
      <div className="absolute bottom-5 right-0 mr-5 flex items-center space-x-4">
        <div
          className="w-12 h-12 hover:scale-105 transition-all ease-in-out duration-200 cursor-pointer rounded-full overflow-hidden bg-gray-300 hover:bg-white flex justify-center items-center"
