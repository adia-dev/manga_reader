...
const Searchbar = () => {
  ...
  const quickSearchBarOpened = useAppSelector(
    (state) => state.quickSearch.active
  );
    ...
    const debounce = setTimeout(async () => {
      if (query.length > 0) {
        const results = await axios.get(
          `http://localhost:5172/manga/${encodeURI(query)}`
        );

        const mappedResults = await Promise.all(
          results.data.data.map(async (manga: any) => {
            let cover = manga.relationships.find(
            ...
            return {
              ...
              rating:
                Math.round(
                  (mangaStats.rating.bayesian + Number.EPSILON) * 100
                ) / 100,
              saved: false,
  ...
  return (
    <div
      className="w-screen h-screen fixed top-0 left-0 z-40 bg-[#000000] bg-opacity-50  flex justify-center"
      style={{ visibility: quickSearchBarOpened ? "visible" : "hidden" }}
    >
      <div
        className="fixed top-0 left-0 w-full h-full bg-[#000000] bg-opacity-50 "
        id="searchbar-background"
      ></div>
      <div
        ...
        style={{
          ...
          transform: quickSearchBarOpened
            ? "scale(1) translateY(0%)"
            : "scale(0.85) translateY(-20%)",
      ...
      >
        <div className="flex justify-center items-center border-b px-5 space-x-2">
          ...
          <input
            ...
            placeholder="Search for a manga, an author, an artist..."
          />
          <div
          ...
          >
            <span className="bg-gray-400 text-gray-100 p-1 rounded-lg">
              esc
            </span>
        ...
        <div className="flex items-center justify-between my-3 px-5">
          <p>Recent</p>
          {/* <BiTrash className="text-2xl text-gray-400 hover:text-red-500 cursor-pointer transition duration-200" /> */}
        </div>
        <div className="flex flex-col overflow-y-scroll max-h-[350px]">
            ...
            recents.map((recent) => (
              ...
              >
                <div className="flex items-start space-x-2">
                  <img
                    src={recent.manga.cover}
                    alt=""
                    className="w-12 aspect-[1/1.5] brightness-75 hover:brightness-105 transition-all duration-500 delay-200 cursor-pointer hover:w-16 object-cover rounded-md border border-black"
                  />
                  <div className="flex flex-col">
                    ...
                    <div className="my-1 flex items-center w-fit space-x-2 bg-gray-900 text-gray-200 p-1 text-xs rounded-md">
                      <div
                        className={`h-2 w-2 bg-${MangaStatusColor(
                          recent.manga.status
                        )}-500 rounded-full`}
                      ></div>
          ...
          {results.map((result: any) => (
            <Link to={`/manga/${result.id}`} key={`result-${result.id}`}>
              <div className="flex items-center justify-between px-5 py-3 border-b hover:bg-gray-100 transition duration-200 cursor-pointer">
                <div className="flex items-start space-x-2">
                  <img
                    src={result.cover}
                    alt=""
